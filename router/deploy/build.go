...
func BuildStart(c *gin.Context) {
    ...
    // build
    workSpace := gostring.JoinStrings(pandora.App.LocalTmpSpace, "/", gostring.Int2Str(build.ApplyId))
    packFile := gostring.JoinStrings(pandora.App.LocalTarSpace, "/", gostring.Int2Str(build.ApplyId), ".tgz")
    ...
    bui, err := buiTask.NewBuild(repo, workSpace, pandora.App.LocalTmpSpace, packFile, p.BuildScript)
    if err != nil {
        render.AppError(c, err.Error())
        return
    }
