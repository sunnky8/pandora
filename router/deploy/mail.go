...
func mailBodyTemplate(msg *MailMessage) string {
    ...
    <style>
        ...
        .pandora-main a:hover {
            color: #40a9ff;
        }
        .pandora-main .strong {
            font-weight: 500;
        }
        .pandora-main .pandora-card {
            background: #fff;
            line-height: 1.8;
        ...
        .pandora-main .btn-primary {
            color: #fff;
            background-color: #409eff;
            border-color: #409eff;
        }
        .pandora-main .btn-primary:hover {
            background: #66b1ff;
            border-color: #66b1ff;
            color: #fff;
        }
        ...
        .pandora-main .underline {
            text-decoration:underline;
        }
        .pandora-main .pandora-cpy {
            padding: 0 20px 10px;
        ...
        .pandora-main .pandora-success {
            color: #52c41a;
        }
        .pandora-main .pandora-failed {
            color: #f5222d;
        }
    </style>
    <div class="pandora-main">
        <div class="pandora-card">
            ...
            <p>
                ...
                {{ if eq .Status 1 }}
                <span class="pandora-success">审核通过</span>，可登录系统进行后续操作。
                {{ else }}
                <span class="pandora-failed">审核不通过</span>，可登录系统查看原因。
                {{ end }}
            </p>
            {{ else if eq .Mode 3 }}
            <p>
                ...
                {{ if eq .Status 1 }}
                <span class="pandora-success">部署成功</span>
                {{ else }}
                <span class="pandora-failed">部署失败</span>
                {{ end }}
                ，可登录系统查看详细日志。
            </p>
            {{ end }}
            <p class="mt50"><a href="{{ .Link }}" class="btn btn-primary" target="_blank">登录Pandora</a></p>
