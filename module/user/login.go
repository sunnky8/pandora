...
import (
    ...
    "github.com/ielepro/pandora/util/goaes"
)
...
func (login *Login) ValidateToken() error {
    ...
    tokenValBytes, err := goaes.Decrypt(pandora.App.CipherKey, authTokenBytes)
    if err != nil {
...
func (login *Login) createToken() error {
    ...
    tokenBytes, err = goaes.Encrypt(pandora.App.CipherKey, []byte(loginRaw))
    if err != nil {
        return err
    }
